...
async def _get(path: str) -> dict:
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        async with session.get(f"{BASE_URL}{path}", timeout=aiohttp.ClientTimeout(total=20)) as resp:
...
async def get_price(country: str, product: str) -> float:
    data = await _get(f"/guest/prices?country={country}&product={product}")
    try:
        operators = data[country][product]
        cheapest = min(op["cost"] for op in operators.values())
        return float(cheapest)
    except (KeyError, ValueError):
        raise FiveSimError("Bu servis/mamlakat uchun narx topilmadi")


async def buy_number(country: str, product: str, operator: str = "any") -> dict:
    data = await _get(f"/user/buy/activation/{country}/{operator}/{product}")
    return {"activation_id": str(data["id"]), "phone": data["phone"]}
...
async def get_status(activation_id: str) -> dict:
    ...
    status_map = {
        "PENDING": "waiting",
        "RECEIVED": "waiting",   # sms received but activation still open
        "FINISHED": "received",
        "CANCELED": "cancelled",
        "BANNED": "cancelled",
    }
